...
import { E2E_TESTING_EMAIL } from "../utils/index.js";
...
export const createUser = async (req, res) => {
  ...
  try {
    ...
    if (email === E2E_TESTING_EMAIL) {
      const result = await StudentUser.findOneAndReplace({ email }, finalData, {
        upsert: true,
        new: true,
      });
    ...
    let batchDoc = await Batch.findById(batch);
    console.log({ batchDoc, batch });
    if (!batchDoc) return res.status(404).json({ message: "Batch not found" });
    batchDoc.members.push({
      _id: result._id,
      name: result.name,
    });
    const updatedBatch = await Batch.findByIdAndUpdate(batch, batchDoc, {
      new: true,
    });
