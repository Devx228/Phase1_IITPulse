...
export const getQuestionById = async function (req, res) {
  try {
    ...
    const question = await MatrixModel.findById(id);
...
export const createQuestion = async function (req, res) {
  ...
  try {
    const result = await MatrixModel.create({ _id: id, ...req.body });
    res
      .status(200)
      .json({
...
export const createQuestionBulk = async function (req, res) {
  try {
    ...
    const result = await MatrixModel.insertMany(req.body);
    res
      .status(200)
      .json({ message: `new questions created` });
  } catch (error) {
...
export const updateQuestion = async function (req, res) {
  try {
    ...
    const response = await MatrixModel.findByIdAndUpdate(
      id,
      {
        correctAnswer, // Option<Range> | Option<Digit>,
        sources,
...
export const deleteQuestion = async function (req, res) {
  try {
    ...
    await MatrixModel.findByIdAndDelete(id);
    res
      .status(200)
      .json({ message: `Matrix Question is deleted with id: ${id}` });
  } catch (error) {
    res.status(501).json({ message: error.message });
  }
};
